...
class OceanDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            ...
            month = np.eye(12, dtype=np.float32)[int(month) - 1]
            # if month in (3, 4, 5):
            ...
            concated = np.concatenate((np.concatenate((np.concatenate((np.concatenate((flattened_sla, flattened_sst)), month)), np.array([lat / 90.0]))), np.array([lon / 180.0])))


            return concated, label, n_interpolate
        elif self.split == "valid":
